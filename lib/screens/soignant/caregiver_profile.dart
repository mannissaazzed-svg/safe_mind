...
class _CaregiverProfileScreenState extends State<CaregiverProfileScreen> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          if (_diseasesList.contains(d?['disease'])) {
            _selectedDisease = d?['disease'];
          }
        });
      }
      ...
      if (patientId != null) {
        final pd = await _supabase
            .from('users')
            .select('full_name, disease, avatar_url')
            .eq('id', patientId)
            .maybeSingle();
  ...
  Future<void> _pickImage() async {
    final source = await showModalBottomSheet<ImageSource>(
      ...
      builder: (_) => SafeArea(
        child: Column(
          ...
          children: [
            ListTile(
              leading: const Icon(Icons.camera_alt, color: Color(0xFF419AFF)),
              title: const Text("Appareil photo"),
              onTap: () => Navigator.pop(context, ImageSource.camera),
            ),
            ListTile(
              leading: const Icon(Icons.photo_library, color: Color(0xFF419AFF)),
    ...
    try {
      ...
      if (mounted) {
        ...
        _snack("Photo mise à jour", isError: false);
      }
    } catch (e) {
  ...
  Future<void> _save() async {
    ...
    if (_selectedDisease == null) {
      _snack('Veuillez choisir la maladie suivie');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _data = {...?_data, 'full_name': name, 'disease': _selectedDisease};
        });
        ...
        if (widget.isFirstTime) {
          Navigator.pushReplacement(
            ...
            MaterialPageRoute(
              builder: (_) =>
                  PatientForm(preselectedDisease: _selectedDisease!),
            ),
          );
        } else {
          ...
          _snack('Profil mis à jour', isError: false);
        }
      }
    } catch (e) {
  ...
  void _snack(String msg, {bool isError = true}) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(msg),
      backgroundColor: isError ? Colors.red : const Color(0xFF419AFF),
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return const Scaffold(
          body: Center(
              child: CircularProgressIndicator(color: Color(0xFF419AFF))));
    }
    ...
    final patientName = (_patientData?['full_name'] as String?)?.isNotEmpty == true
        ? _patientData!['full_name'] as String
        : 'Patient (non nommé)';
    final patientDisease = (_patientData?['disease'] as String?)?.isNotEmpty == true
        ? _patientData!['disease'] as String
        : 'Non spécifiée';
              ...
              background: Container(
                decoration: const BoxDecoration(
                  gradient: LinearGradient(
                      colors: [Color(0xFF419AFF), Color(0xFF185FA5)]),
                ),
                child: Column(
                            ...
                            child: CircleAvatar(
                              ...
                              child: (_localImage == null && avatarUrl == null)
                                  ? const Icon(Icons.person,
                                      size: 50, color: Color(0xFF419AFF))
                                  : null,
                            ),
                          ),
                          Positioned(
                            ...
                            child: Container(
                              ...
                              child: const Icon(Icons.camera_alt,
                                  color: Color(0xFF419AFF), size: 20),
                            ),
                          ),
                        ],
                      ),
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                if (widget.isFirstTime) ...[
                  ...
                  const Text(
                      "Veuillez compléter vos informations de soignant.",
  ...
  children: [
    ...
    SizedBox(
      ...
      child: ElevatedButton.icon(
        ...
        label: Text(
          _isSaving ? 'Enregistrement...' : 'Sauvegarder le nom',
          style: const TextStyle(
            color: Colors.white,
            fontWeight: FontWeight.bold,
          ),
            ...
            borderRadius: BorderRadius.circular(12),
                ...
                _card(
                  ...
                  child: Column(
                    children: [
                      const Text(
                        'Partagez ce code avec votre patient pour qu\'il puisse vous lier',
                  ...
                  child: isLinked
                      ? ListTile(
                          ...
                          leading: CircleAvatar(
                            backgroundImage:
                                _patientData!['avatar_url'] != null
                                    ? NetworkImage(
                                        _patientData!['avatar_url'])
                                    : null,
                            child: _patientData!['avatar_url'] == null
                                ? const Icon(Icons.person)
                                : null,
                          ),
