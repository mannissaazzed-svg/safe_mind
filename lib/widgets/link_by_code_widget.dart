...
class _LinkByCodeWidgetState extends State<LinkByCodeWidget> {
  ...
  Future<void> _link() async {
    ...
    try {
      ...
      final targetRole =
          widget.currentRole == 'caregiver' ? 'patient' : 'caregiver';
      ...
      for (final u in list) {
        ...
        if (uid.length >= 8 &&
            uid.substring(0, 8).toUpperCase() == code) {
      ...
      if (match == null) {
        setState(() => _error = l.linkError);
        return;
      }

      final targetId   = match['id'] as String;
      final targetName = match['full_name'] as String? ?? '';

      await widget.supabase
          .from('users')
          .update({'linked_to': targetId}).eq('id', myId);

      await widget.supabase
          .from('users')
          .update({'linked_to': myId}).eq('id', targetId);
      ...
      final l = AppLocalizations.of(context)!;
      setState(() => _error = '${l.linkError}: ${e.toString()}');
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final color       = isCaregiver
        ? const Color(0xFF419AFF)
        : const Color(0xFF1D9E75);
    final hint = isCaregiver ? l.codePatient : l.codeCaregiver;
            ...
            Expanded(
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  counterText: '',
                  prefixIcon:
                      Icon(Icons.vpn_key_outlined, color: color),
